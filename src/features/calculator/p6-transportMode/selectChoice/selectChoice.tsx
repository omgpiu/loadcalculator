...
import {remainderCargoType, tr_ModeActions} from '../p6-reducer';
import {v1} from 'uuid';
...
export const SelectChoice: React.FC<PropsType> = React.memo(({transports, totalCargoValue}) => {
    ...
    const addClickHandler = () => {
        ...
        if (selectTransportId) {
            const selectTransport = transports.find(el => el.id === selectTransportId)
            selectTransport && dispatch(tr_ModeActions.addSelectedTransportAC({selectTransport}))
        }
